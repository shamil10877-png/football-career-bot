...
STADIUM_UPGRADES = [{"name": "Маленький", "cost": 0, "income": 0}, {"name": "Средний", "cost": 200000, "income": 10000}, {"name": "Большой", "cost": 500000, "income": 25000}, {"name": "Супер", "cost": 1500000, "income": 50000}]
PREDICTIONS = [{"match": "Реал Мадрид vs Барселона", "odds": {"1": 2.5, "X": 3.0, "2": 2.8}}]
STOCKS = [{"name": "Nike", "price": 100}, {"name": "Adidas", "price": 80}]
...
def get_clubs_by_rating(rating):
    if rating >= 85:
        return [("Реал Мадрид", 300000), ("Барселона", 280000)]
    elif rating >= 75:
        return [("Челси", 180000), ("Ювентус", 170000)]
    elif rating >= 65:
        return [("Вест Хэм", 100000), ("Валенсия", 90000)]
    else:
        return [("Кристал Пэлас", 40000), ("Хоффенхайм", 35000)]
...
async def help_cmd(message: Message):
    ...
    text = "📖 <b>Команды</b>\n\n"
    text += "/profile, /train, /match, /rest, /season, /calendar\n"
    text += "/transfers, /agent, /shop, /stats, /trophies, /news, /table, /ucl, /contract, /sponsor, /national\n"
    text += "/cars, /house, /family, /business, /history, /donate, /bonus, /achievements, /leaderboard, /penalty, /casino, /tasks, /referral, /reminder, /ping, /myid, /give\n"
...
async def shop_cb(call: CallbackQuery):
    ...
    if key == "boots":
        ...
        if p.get("injured"): p["injured"] = False; p["injury_days"] = 0    elif key == "nutrition":
