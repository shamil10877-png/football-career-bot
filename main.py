...
async def match_cmd(message: Message):
    ...
    if mvp:
        p["mvps"] += 1
    gain = goals * 0.35 + assists * 0.20 + (0.50 if mvp else 0) + (0.30 if result == "win" else 0.10 if result == "draw" else 0)
    p["rating"] = round(min(99, p["rating"] + gain), 1)
    reward = goals * 8000 + assists * 5000 + (15000 if mvp else 0) + (5000 if result == "win" else 0)
    p["money"] += reward
    p["fitness"] = max(0, p["fitness"] - random.randint(10, 20))
    p["followers"] += random.randint(50, 300) + goals * 80 + assists * 40 + (300 if mvp else 0) + (200 if result == "win" else 0)
    p["week"] += 1
    ...
    if p["week"] >= 38:
        ...
        if p["age"] >= 41:
            p["retired"] = True
            save(message.from_user.id, p)
            return await message.answer("🔚 Карьера завершена!")
    save(message.from_user.id, p)
...
async def shop_cb(call: CallbackQuery):
    ...
    p["money"] -= item["cost"]
    if call.data.split(":")[1] == "boots":
        p["pace"] = min(99, p["pace"] + 2)
    elif call.data.split(":")[1] == "equipment":
...
async def casino_roulette(call: CallbackQuery):
    ...
    await call.message.edit_text("♠️ Ставка 20,000", reply_markup=keyboard)
...
async def roulette_cb(call: CallbackQuery):
    ...
    p["money"] += win
    save(uid, p)
    await call.message.edit_text(f"♠️ Выпало: {bot_choice}\n🏆 {money(win)}\n💵 {money(p['money'])}")
...
async def staff(message: Message):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"⬆️ {money([50000, 200000, 500000][level])}", callback_data="staff:upgrade")] if level < 3 else []
    ])
...
async def seasons(message: Message):
    ...
    await message.answer(f"📊\n{'\n'.join([f'{i+1}. Сезон {s.get(\"season\", 0)} — ⭐{s.get(\"rating\", 0)}' for i, s in enumerate(stats[-5:])]) or 'Нет'}")
