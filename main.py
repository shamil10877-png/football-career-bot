...
async def business(message: Message):
    ...
    cost = 2000000
    if p["money"] < cost:
        await message.answer(f"💸 Нужно {money(cost)}.")
        return
    ...
    await message.answer("🏢 Ты открыл собственный бизнес!")
...
async def halloffame(message: Message):
    ...
    trophies = p["ucl"] + p["league_titles"] + p["worldcup"] + p["ballon"]
...
🏆 Трофеев: {p['ucl'] + p['league_titles'] + p['worldcup'] + p['ballon']}
⚽ Голов: {p['goals']}
🎯 Ассистов: {p['assists']}
