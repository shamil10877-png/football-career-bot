...
async def start(message: Message):
    ...
    if p:
        await message.answer(
            ...
            "📅 Текущая неделя: " + str(p.get("week", 1)) + "\n"
...
async def match_cmd(message: Message):
    ...
    gain = goals * 0.40 + assists * 0.25 + (0.50 if mvp else 0)
    p["rating"] = round(min(99, p["rating"] + gain), 1)

    reward = goals * 15000 + assists * 9000 + (25000 if mvp else 0)
    p["money"] += reward
    p["market_value"] = int(p["rating"] * 120000)
    p["fitness"] = max(0, p["fitness"] - random.randint(10, 20))

    followers_gain = random.randint(100, 600) + goals * 100 + assists * 50
    if mvp:
        followers_gain += 500
    ...
    news = random.choice([
        f"📰 {p['name']} забил {goals} гола(ов) и отдал {assists} ассистов!",
        f"📰 {p['name']} признан лучшим игроком матча!",
        f"📰 Болельщики в восторге от игры {p['name']}!",
        f"📰 {p['name']} продолжает удивлять своей формой!"
    ])
...
async def season(message: Message):
    ...
    await message.answer(
        f"""📅 <b>Сезон {p['season'] - 1} завершён!</b>
...
👥 Подписчики: {p['followers']}
🎂 Возраст: {p['age']}
🏆 Трофеев за сезон: {len(p['trophies'])}
"""
    )
...
async def cars(message: Message):
    ...
    car, price = random.choice(cars_list)
...
async def house(message: Message):
    ...
    await message.answer("💸 Денег пока недостаточно.")
...
👥 Подписчики: {p['followers']:,}
⭐ Репутация: {p.get('reputation', 50)}/100
"""
    )
...
async def reputation(message: Message):
    ...
    p["reputation"] = max(0, min(100, p.get("reputation", 50) + change))
...
📊 Изменение: {change if change >= 0 else ''}{change}
📈 Текущая: {p['reputation']}/100
"""
    )
...
async def halloffame(message: Message):
    ...
    trophies = p["ucl_career"] + p["league_titles_career"] + p["worldcup_career"] + p["ballon_career"]
...
async def retire(message: Message):
    ...
    if p.get("retired"):
        await message.answer("❌ Карьера уже завершена.")
        return

    if p["age"] < 41:
        await message.answer("❌ Ты не можешь завершить карьеру раньше 41 года!")
        return
...
🏆 Трофеев: {p['ucl_career'] + p['league_titles_career'] + p['worldcup_career'] + p['ballon_career']}
⚽ Голов: {p['goals_career']}
🎯 Ассистов: {p['assists_career']}

Ты навсегда вписал своё имя в историю футбола! 🏅
"""
    )
...
async def table(message: Message):
    ...
    points = [random.randint(20, 90) for _ in range(len(table_list))]
    points[table_list.index(p["club"])] = random.randint(60, 95)
...
async def agent(message: Message):
    ...
    if p.get("agent") != "Нет":
        await message.answer(f"🧑‍💼 Твой агент: <b>{p['agent']}</b>")
        return
...
async def hire_agent(call: CallbackQuery):
    uid = call.from_user.id
    p = load(uid)
    if not p:
        await call.answer("❌ Сначала создай карьеру!", show_alert=True)
