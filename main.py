Отлично! Вы хотите серьёзно прокачать бота. Давайте я поэтапно добавлю все эти функции. Начну с самых важных механик.

Вот обновлённый код с новыми функциями. Заменяйте им свой текущий код:
...
def get_club_level(rating: int) -> tuple:
    """Возвращает (уровень клуба, название лиги)"""
    if rating >= 85:
        return 4, "Топ-клуб"
    elif rating >= 75:
...
def get_club_offers(rating: int) -> list:
    """Генерирует предложения от клубов в зависимости от рейтинга"""
    clubs = []
...
async def cmd_match(message: Message):
    ...
    # Симуляция матча
    rating_modifier = (p["rating"] - 60) / 20  # -2 до +2
    ...
    # Изменение рейтинга от голов и ассистов
    rating_change = 0
    if goals > 0:
        rating_change += goals * 0.5
        if goals >= 3:
            rating_change += 1  # хет-трик
    if assists > 0:
        rating_change += assists * 0.3
    if is_mvp:
        rating_change += 1.5
    
    # Случайные колебания формы
...
async def cmd_train(message: Message):
    ...
    # Выбор улучшаемой характеристики
...
async def cmd_season(message: Message):
    ...
    # Симуляция сезона
    # Трофеи зависят от рейтинга
    ucl_chance = max(0, (p["rating"] - 70) / 30)  # 0 до 1
    ucl = 1 if random.random() < ucl_chance else 0
    
    league_chance = max(0, (p["rating"] - 60) / 40)  # 0 до 1
    ...
    # Обновление зарплаты и стоимости
...
⚽ Голы: {p['goals']} (всего: {sum(hist.get('goals', 0) for hist in p['club_history']) + p['goals']})
🎯 Ассисты: {p['assists']}
🎮 Матчей: {p['matches']}
⭐ MVP: {p['mvps']}
...
async def cmd_table(message: Message):
    ...
    # Добавляем клуб игрока если его нет
...
async def cmd_sponsor(message: Message):
    ...
    # Поиск спонсора в зависимости от рейтинга
    if p["rating"] >= 85:
        sponsors = [
...
async def cmd_leaderboard(message: Message):
    # В реальном боте здесь была бы база данных всех игроков
    # Для демонстрации показываем топ-10 сгенерированных игроков
    top_players = [
