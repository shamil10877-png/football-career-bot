...
CUP_CLUBS = ["Реал Мадрид", "Барселона", "Манчестер Сити", "Бавария", "ПСЖ", "Ливерпуль", "Челси", "Арсенал", "Ювентус", "Атлетико"]
CASES = [
...
MERCH_ITEMS = [{"name": "👕 Футболка", "cost": 25000, "income": 5000}, {"name": "🧢 Кепка", "cost": 10000, "income": 2000}, {"name": "🧣 Шарф", "cost": 15000, "income": 3000}]
STADIUM_UPGRADES = [{"name": "Маленький стадион", "cost": 0, "income": 0}, {"name": "Средний стадион", "cost": 200000, "income": 10000}, {"name": "Большой стадион", "cost": 500000, "income": 25000}, {"name": "Супер стадион", "cost": 1500000, "income": 50000}, {"name": "Стадион имени тебя", "cost": 5000000, "income": 150000}]
PREDICTIONS = [{"match": "Реал Мадрид vs Барселона", "odds": {"1": 2.5, "X": 3.0, "2": 2.8}}, {"match": "Манчестер Сити vs Ливерпуль", "odds": {"1": 2.2, "X": 3.2, "2": 3.0}}, {"match": "Бавария vs Боруссия", "odds": {"1": 1.8, "X": 3.5, "2": 4.0}}]
STOCKS = [{"name": "Nike", "price": 100}, {"name": "Adidas", "price": 80}, {"name": "Puma", "price": 60}, {"name": "EA Sports", "price": 150}]
...
async def transfers(message: Message):
    ...
    await message.answer(f"✈️ <b>Трансферные предложения</b>\nТекущий клуб: {p['club']}\n⭐ Рейтинг: {p['rating']}", reply_markup=keyboard)
...
async def transfer_cb(call: CallbackQuery):
    ...
    save(uid, p)
    await call.message.edit_text(f"✅ <b>Трансфер совершён!</b>\nТы покинул {old_club} и перешёл в {club}!\n💵 Новая зарплата: {money(salary)}/нед")
...
async def winter_cb(call: CallbackQuery):
    ...
    offers = random.sample(clubs, min(3, len(clubs)))
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"🏟 {club} — {money(sal)}/нед", callback_data=f"winter_transfer:{club}:{sal}")] for club, sal in offers
    ] + [[InlineKeyboardButton(text="❌ Остаться", callback_data="winter:no")]])
    await call.message.edit_text(f"❄️ <b>Зимнее трансферное окно!</b>\nТекущий клуб: {p['club']}", reply_markup=keyboard)
...
async def winter_transfer_cb(call: CallbackQuery):
    ...
    p["winter_transfer"] = True
    save(uid, p)
    await call.message.edit_text(f"✅ <b>Зимний трансфер!</b>\nТы перешёл из {old_club} в {club}!\n💵 Новая зарплата: {money(salary)}/нед")
...
async def agent(message: Message):
    ...
    if p.get("agent") != "Нет":
        await message.answer(f"🧑‍💼 <b>Твой агент</b>\nАгент: {p['agent']}\n💰 Деньги: {money(p['money'])}", reply_markup=keyboard)
        return
    await message.answer(f"🧑‍💼 <b>Выбери агента</b>\n💰 Деньги: {money(p['money'])}", reply_markup=keyboard)
...
async def shop(message: Message):
    ...
    items = {
        ...
        "nutrition": {"name": "🥗 Персональный диетолог", "cost": 15000, "stat": "+5 к форме"},
...
async def cars(message: Message):
    ...
    await message.answer(f"🚗 <b>Автосалон</b>\n💰 Деньги: {money(p['money'])}\n🚘 Твоя машина: {p.get('car', 'Нет')}", reply_markup=keyboard)
...
async def house(message: Message):
    ...
    if p.get("house") != "Нет":
        ...
        await message.answer(f"🏠 <b>Твой дом</b>\n🏡 Дом: {p['house']}\n💰 Доход: {money(p.get('rental_income', 0))}/нед", reply_markup=keyboard)
        return
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
    ...
    await message.answer(f"🏠 <b>Недвижимость</b>\n💰 Деньги: {money(p['money'])}\n🏠 Твой дом: {p.get('house', 'Нет')}", reply_markup=keyboard)
...
async def business(message: Message):
    ...
    if not p:
        ...
        return
    if p.get("business"):
        await message.answer(f"🏢 <b>Твой бизнес</b>\n🏢 {p.get('business_type', 'Нет')}\n💰 Доход: {money(p.get('business_income', 0))}/нед\n💵 Всего: {money(p['money'])}")
        return
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
    ...
    await message.answer(f"🏢 <b>Бизнес</b>\n💰 Деньги: {money(p['money'])}", reply_markup=keyboard)
...
async def add_money(message: Message):
    ...
    if len(args) < 3:
        ...
        return    username = args[1].replace("@", "")
    amount = int(args[2])
    for file in os.listdir(SAVE_DIR):
        if file.endswith(".json"):
            with open(os.path.join(SAVE_DIR, file), "r") as f:
...
async def bonus_cmd(message: Message):
    ...
    if p.get("bonus_received", False):
        await message.answer("❌ Ты уже получил бонус!")
        return
    try:
...
async def penalty_cb(call: CallbackQuery):
    ...
    if user_choice == bot_choice:
        ...
        p["money"] += reward
        save(uid, p)
        await call.message.edit_text(f"⚽ <b>ГОЛ!</b> 🎉\n💰 +{money(reward)}")
    else:
        await call.message.edit_text(f"⚽ <b>МИМО!</b> ❌")
...
async def casino(message: Message):
    ...
    await message.answer(f"🎰 <b>Казино</b>\n💰 Баланс: {money(p['money'])}", reply_markup=keyboard)
...
async def referral(message: Message):
    ...
    text = f"🔗 <b>Реферальная система</b>\n📋 Твоя ссылка: <code>https://t.me/GoalTimeNews_bot?start={message.from_user.id}</code>\n👥 Приглашённых: {len(refs)}"
    if refs:
        text += f"\n📋 Список: {', '.join(refs[:5])}"
    await message.answer(text)
...
async def reminder(message: Message):
    ...
    if (now - last).days >= 1:
        ...
        save(message.from_user.id, p)
        await message.answer("⏰ <b>Напоминание!</b>\n+10,000 € за активность! 🎉")
    else:
        await message.answer("⏰ <b>Напоминание</b>\nБонус уже получен сегодня. Приходи завтра!")
...
async def successful_payment(message: Message):
    ...
    if p:
        p["money"] += amount
        save(message.from_user.id, p)
        await message.answer(f"✅ <b>Спасибо за поддержку!</b> ❤️\n⭐ {stars} звёзд\n💰 +{money(amount)}\n💵 Баланс: {money(p['money'])}")
    else:
        await message.answer(f"✅ Спасибо! ⭐ {stars} звёзд получено! Создай карьеру через /create.")
...
async def cup(message: Message):
    ...
    if not p:
        await message.answer("❌ Сначала создай карьеру!")
        return
    if p.get("retired") or p.get("injured") or p["fitness"] < 15:
...
async def stadium(message: Message):
    ...
    await message.answer(
        f"🏟️ <b>Стадион</b>\nТекущий: {current['name']}\n💰 Доход: {money(current['income'])}/нед\nСледующий: {next_upgrade['name']}\n💰 Стоимость: {money(next_upgrade['cost'])}",
...
async def stadium_cb(call: CallbackQuery):
    ...
    save(uid, p)
    await call.message.edit_text(f"✅ Стадион улучшен!\n🏟️ {next_upgrade['name']}\n💰 Доход: {money(next_upgrade['income'])}/нед")
...
async def merch(message: Message):
    ...
    if merch_items:
        ...
        for item in merch_items:
            text += f"{item['name']} — {money(item['income'])}/нед\n"
            total += item.get("income", 0)
        text += f"💰 Общий доход: {money(total)}/нед"
        await message.answer(text)
        return
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
    ...
    await message.answer(f"👕 <b>Магазин мерча</b>\n💰 Деньги: {money(p['money'])}", reply_markup=keyboard)
...
async def predict(message: Message):
    ...
    await message.answer(f"🔮 <b>Прогноз</b>\n⚔️ {match['match']}\n💰 Ставка: 10,000 €", reply_markup=keyboard)
...
async def stocks(message: Message):
    ...
    if not stocks:
        ...
        await message.answer(f"📈 <b>Акции</b>\n💰 Деньги: {money(p['money'])}", reply_markup=keyboard)
        return
    text = "📈 <b>Твои акции</b>\n"
    total = 0
    for s in stocks:
    ...
    text += f"💰 Общая стоимость: {money(total)}\n💵 Денег: {money(p['money'])}"
    await message.answer(text)
...
async def become_coach_cb(call: CallbackQuery):
    ...
    if call.data.split(":")[1] == "yes":
        ...
        p["coach_money"] = 50000
        save(uid, p)
        await call.message.edit_text(f"🧑‍🏫 <b>Поздравляем! Ты стал тренером!</b>\n🏟 Команда: {p['coach_team']}\n⭐ Рейтинг тренера: {p['coach_rating']}\n💰 Денег: {money(p['coach_money'])}")
    else:
        await call.message.edit_text("🔚 Ты завершил игру. Спасибо! 🏆")

@dp.message(Command("coach_profile"))
async def coach_profile(message: Message):
    p = load(message.from_user.id)
    if not p or not p.get("is_coach"):
        await message.answer("❌ Ты не тренер! Заверши карьеру игрока.")
        return
    text = f"🧑‍🏫 <b>Профиль тренера</b>\n👤 Имя: {p['name']}\n⭐ Рейтинг: {p['coach_rating']}\n🏟 Команда: {p.get('coach_team', 'Нет')}\n📊 Матчи: {p.get('coach_matches', 0)} ({p.get('coach_wins', 0)}-{p.get('coach_draws', 0)}-{p.get('coach_losses', 0)})\n🏆 Трофеев: {len(p.get('coach_trophies', []))}\n💰 Денег: {money(p.get('coach_money', 0))}\n🧠 Тактика: {p.get('coach_tactics', 'Сбалансированная')}"
    await message.answer(text)
...
async def team(message: Message):
    ...
    text = f"🏟 <b>{p.get('coach_team', 'Команда')}</b>\n👥 Игроков: {len(players)}\n⭐ Средний рейтинг: {sum(p.get('rating', 60) for p in players)//max(1, len(players)) if players else 60}\n📊 Матчи: {p.get('coach_matches', 0)}\n🏆 Трофеев: {len(p.get('coach_trophies', []))}"
    if players:
        text += "\n📋 Состав:\n" + "\n".join([f"{i+1}. {pl.get('name', 'Игрок')} — ⭐{pl.get('rating', 60)}" for i, pl in enumerate(players[:5])])
    await message.answer(text)

@dp.message(Command("train_team"))
async def train_team(message: Message):
    p = load(message.from_user.id)
    if not p or not p.get("is_coach") or not p.get("coach_players"):
        await message.answer("❌ Ты не тренер или нет игроков!")
        return
    for pl in p["coach_players"][:5]:
...
async def match_team(message: Message):
    ...
    save(message.from_user.id, p)
    result_text = "🏆 Победа!" if result == "win" else "🤝 Ничья" if result == "draw" else "❌ Поражение"
    await message.answer(f"🏟 Матч завершён!\n📊 Результат: {result_text}\n⚔️ Соперник: {opp}\n⭐ Рейтинг: {p['coach_rating']}\n💰 Денег: {money(p.get('coach_money', 0))}")
...
async def transfer_players(message: Message):
    ...
    if not players:
        await message.answer("📭 Нет игроков.")
        return
    text = "🔄 <b>Трансферы</b>\n" + "\n".join([f"{i+1}. {pl.get('name', 'Игрок')} — ⭐{pl.get('rating', 60)} — 💰{money(pl.get('price', 50000))}" for i, pl in enumerate(players[:5])])
    text += f"\n💰 Денег: {money(p.get('coach_money', 0))}"
    await message.answer(text)
...
async def squad(message: Message):
    ...
    if not players:
        await message.answer("📭 Нет игроков.")
        return
    text = f"📋 <b>Состав</b>\n" + "\n".join([f"{i+1}. {pl.get('name', 'Игрок')} — ⭐{pl.get('rating', 60)}" for i, pl in enumerate(players[:10])])
    await message.answer(text)
...
async def coach_trophies(message: Message):
    ...
    await message.answer(f"🏆 <b>Трофеи тренера</b>\n{'\n'.join(trophies) if trophies else 'Пока нет'}")
...
async def staff(message: Message):
    ...
    bonuses = [0, 5, 10, 20]
    if level >= 3:
        await message.answer(f"🏠 <b>Штаб</b>\nУровень {level+1} — максимальный!")
        return
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"⬆️ Улучшить до уровня {level+2} — {money(costs[level+1])}", callback_data="staff:upgrade")]
    ])
    await message.answer(f"🏠 <b>Штаб</b>\nУровень: {level+1}\nБонус: +{bonuses[level]}% к тренировкам\nСтоимость улучшения: {money(costs[level+1])}", reply_markup=keyboard)
...
async def menu_cb(call: CallbackQuery):
    ...
    menus = {
        "profile": "👤 <b>Профиль</b>\n/profile",
        "game": "⚽ <b>Игровой процесс</b>\n/train, /match, /rest, /season, /calendar, /cup, /club_world_cup",
        "stats": "📊 <b>Статистика</b>\n/stats, /trophies, /table, /leaderboard, /seasons",
        "career": "🔄 <b>Карьера</b>\n/transfers, /agent, /contract, /sponsor, /history",
        "shop": "🛒 <b>Магазин</b>\n/shop, /cars, /house, /business, /merch, /stadium, /cases",
        "social": "👤 <b>Социальное</b>\n/news, /family, /friends, /rename, /setemoji",
        "bonus": "⭐ <b>Бонусы</b>\n/daily, /bonus, /tasks, /referral, /reminder, /predict, /stocks",
        "casino": "🎰 <b>Казино</b>\n/casino, /penalty",
