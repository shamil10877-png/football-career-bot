...
# ============ ОСТАЛЬНЫЕ КОМАНДЫ (ТРЕНЕР И Т.Д.) ============
...
async def casino_slots(call: CallbackQuery):
    ...
    win = 0    if result[0] == result[1] == result[2]:
        win = bet * 10
...
async def menu_cb(call: CallbackQuery):
    ...
    await call.message.edit_text(menus.get(action, "❌ Неизвестный раздел"), reply_markup=main_menu_keyboard(p
